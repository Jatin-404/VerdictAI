    ...
    try:
        for chunk in data.chunks:
            ...
            INSERT INTO legal_chunks (
                ...
                filename, ingestion_timestamp, judgment_id,
                court, court_level, decision_date, domain,
                bench, jurisdiction, embedding
            )
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            ON CONFLICT (chunk_id) DO NOTHING
            """, (
            str(uuid.uuid4()),
            chunk.document_id, chunk.chunk_text, chunk.chunk_index,
            chunk.filename, chunk.ingestion_timestamp,
            chunk.judgment_id, chunk.court, chunk.court_level,
            chunk.decision_date, chunk.domain,
            chunk.bench, chunk.jurisdiction,
            chunk.vector
            ))
            stored_count += 1
...
def get_all_chunks():
    ...
    try:
        cursor.execute("SELECT * FROM legal_chunks")
...
def get_chunks_by_document(document_id: str):
    ...
    try:
        cursor.execute(
            "SELECT * FROM legal_chunks WHERE document_id = %s",
...
def get_all_documents():
    ...
    try:
        ...
        cursor.execute("""
            SELECT document_id, judgment_id, court, court_level, domain, filename, COUNT(*) as total_chunks
            FROM legal_chunks
            GROUP BY document_id, judgment_id, court, court_level, domain, filename
            """)
        rows = cursor.fetchall()
